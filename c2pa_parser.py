...
- `c2pa-python` is an OPTIONAL dependency. If it isn't installed, parsing degrades to a clear,
...
Usage
-----
    python3 c2pa_parser.py path/to/file.wav
"""
...
def parse_c2pa_file(path: str) -> C2paParseResult:
    ...
    if c2pa is None:
        return C2paParseResult(
            source_path=path,
            warnings=[
                "c2pa-python is not installed; run "
                "'python3 -m pip install c2pa-python' to enable C2PA parsing."
            ],
        )
